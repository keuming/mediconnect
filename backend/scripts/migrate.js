    ...
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS utilisateurs (
        ...
        role        VARCHAR(25)  NOT NULL CHECK (role IN (
                      ...
                      'admin','assureur','laboratoire','imagerie',
                      'medecin_independant'
                    )),
      ...
      --  IMAGERIE MEDICALE
      -- ════════════════════════════════════════
      CREATE TABLE IF NOT EXISTS imageries (
        id              UUID         PRIMARY KEY DEFAULT uuid_generate_v4(),
        user_id         UUID         NOT NULL REFERENCES utilisateurs(id) ON DELETE CASCADE,
        nom             VARCHAR(200) NOT NULL,
        email           VARCHAR(255),
      ...
      -- ════════════════════════════════════════
      --  MEDECINS (rattaches a une clinique)
      -- ════════════════════════════════════════
      ...
      -- ════════════════════════════════════════
      --  MEDECINS INDEPENDANTS
      --  Exercent en cabinet prive, sans clinique
      --  Ont un compte utilisateur propre (user_id)
      --  Ont leur propre position geo pour etre
      --  trouves par les patients a proximite
      -- ════════════════════════════════════════
      CREATE TABLE IF NOT EXISTS medecins_independants (
        id             UUID         PRIMARY KEY DEFAULT uuid_generate_v4(),
        user_id        UUID         NOT NULL REFERENCES utilisateurs(id) ON DELETE CASCADE,
        prenom         VARCHAR(100) NOT NULL,
        nom            VARCHAR(100) NOT NULL,
      ...
      -- ════════════════════════════════════════
      --  RENDEZ-VOUS
      --  Supporte clinique ET medecin independant
      -- ════════════════════════════════════════
      CREATE TABLE IF NOT EXISTS rendez_vous (
        ...
        medecin_independant_id UUID       REFERENCES medecins_independants(id),
        ...
        notes                 TEXT,
        created_at            TIMESTAMPTZ DEFAULT NOW(),
        CONSTRAINT chk_rdv_destination CHECK (
          clinique_id IS NOT NULL OR medecin_independant_id IS NOT NULL
        )
      );
      ...
      CREATE TABLE IF NOT EXISTS transactions_caisse (
        ...
        mode       VARCHAR(50) DEFAULT 'Especes',
      ...
      CREATE INDEX IF NOT EXISTS idx_rdv_medindep       ON rendez_vous(medecin_independant_id);
      ...
      CREATE INDEX IF NOT EXISTS idx_consult_medindep   ON consultations(medecin_independant_id);
      ...
      -- ════════════════════════════════════════
      --  INDEX GEOGRAPHIQUES PostGIS (GIST)
      -- ════════════════════════════════════════
      CREATE INDEX IF NOT EXISTS idx_geo_cliniques      ON cliniques             USING GIST(geo_point);
      CREATE INDEX IF NOT EXISTS idx_geo_pharmacies     ON pharmacies            USING GIST(geo_point);
      CREATE INDEX IF NOT EXISTS idx_geo_livreurs       ON livreurs              USING GIST(geo_point);
      CREATE INDEX IF NOT EXISTS idx_geo_assureurs      ON assureurs             USING GIST(geo_point);
      CREATE INDEX IF NOT EXISTS idx_geo_patients       ON patients              USING GIST(geo_point);
      CREATE INDEX IF NOT EXISTS idx_geo_laboratoires   ON laboratoires          USING GIST(geo_point);
      CREATE INDEX IF NOT EXISTS idx_geo_imageries      ON imageries             USING GIST(geo_point);
      CREATE INDEX IF NOT EXISTS idx_geo_med_independ   ON medecins_independants USING GIST(geo_point);
    `);

    await client.query('COMMIT');
    console.log('✅ Migration terminee — toutes les tables creees');
  } catch (err) {
