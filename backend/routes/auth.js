...
const genToken = (id, role, extra = {}) => jwt.sign({ id, role, ...extra }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '7d' });
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const token = genToken(userId, role);
...
router.post('/login', async (req, res) => {
  ...
  try {
    const result = await query('SELECT * FROM utilisateurs WHERE email=$1 AND is_active IS NOT false', [email]);
    ...
    const token = genToken(user.id, user.role, {
      clinique_id: user.clinique_id || null,
