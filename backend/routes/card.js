    ...
    if (!card.rows.length) {
      ...
      if (!request.rows.length) {
        ...
        const patient = await db(
          'SELECT id, prenom, nom, telephone, ville, groupe_sanguin, allergies, antecedents_critiques, traitements_sensibles FROM patients WHERE UPPER(code_secret)=UPPER($1)',
          [numero_carte]
        );
        ...
        return res.json({
          ...
          data: {
            ...
            patient: {
              ...
              antecedents_critiques: p.antecedents_critiques, traitements_sensibles: p.traitements_sensibles,
