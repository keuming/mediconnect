...
if (!process.env.JWT_SECRET) {
  console.warn('[WARN] JWT_SECRET manquant — utilisation valeur par défaut');
}
...
// ════════════════════════════════════════════════════════════════
// CORS — ABSOLUMENT EN PREMIER (avant tout middleware)
// ════════════════════════════════════════════════════════════════
...
app.use('/api/', rateLimit({
  windowMs: 60 * 1000, max: 500, standardHeaders: true, legacyHeaders: false,
  skip: (req) => req.method === 'OPTIONS',
}));
...
// Routes optionnelles (si les fichiers existent)
const optionalRoutes = [
  ['/api/pharmacies',   './routes/pharmacies'],
  ['/api/commandes',    './routes/commandes'],
  ['/api/livreurs',     './routes/livreurs'],
  ['/api/notifications','./routes/notifications'],
];
optionalRoutes.forEach(([path, routeFile]) => {
  try {
    app.use(path, require(routeFile));
  } catch (e) {
    // Route non disponible — ignorée silencieusement
  }
});
...
app.use((err, req, res, next) => { // eslint-disable-line no-unused-vars
  ...
  res.status(status).json({
    success: false,
    message: isProd && status >= 500 ? 'Erreur interne' : err.message,
  });
});
