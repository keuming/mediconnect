  ...
  const submitRDV = async () => {
    ...
    try {
      ...
      if (clinique.id.startsWith('demo-') || medecin.id.startsWith('md-')) {
        ...
        nav('/confirmation', { state: { clinique, medecin, creneau, patient, specialite, reference: 'MC-RDV-' + Math.random().toString(36).slice(2,8).toUpperCase() } });
        return;
      }
      ...
      toast.success('RDV confirmé !');
      nav('/confirmation', { state: { clinique, medecin, creneau, patient, specialite, reference: data.data.reference, code_secret: data.data.code_secret } });
    } catch (err) {
