...
function PageGestionRDV() {
  ...
  const [filtreClinik,   setFiltreClinik]   = useState("tous");

  // Calculs globaux
  const rdvTotal    = rdvs.length;
  const rdvTenus    = rdvs.filter(r=>r.statut==="confirme"||r.statut==="confirmed"||r.statut==="tenu").length;
  const rdvAnnules  = rdvs.filter(r=>r.statut==="annule"||r.statut==="cancelled").length;
  const rdvEnCours  = rdvs.filter(r=>r.statut==="en_attente"||r.statut==="pending").length;

  // Calculs par clinique
  const statsParClinique = cliniques.map(c => {
    const rdvclin   = rdvs.filter(r=>String(r.clinique_id)===String(c.id));
    return {
      ...c,
      total:   rdvclin.length,
      tenus:   rdvclin.filter(r=>r.statut==="confirme"||r.statut==="confirmed"||r.statut==="tenu").length,
      annules: rdvclin.filter(r=>r.statut==="annule"||r.statut==="cancelled").length,
      enCours: rdvclin.filter(r=>r.statut==="en_attente"||r.statut==="pending").length,
    };
  });
  ...
  const rdvsFiltres = rdvs.filter(r => {
    const matchStatut = filtreStatut === "tous" ? true
      : filtreStatut === "tenus"   ? (r.statut==="confirme"||r.statut==="confirmed"||r.statut==="tenu")
      : filtreStatut === "annules" ? (r.statut==="annule"||r.statut==="cancelled")
      : (r.statut==="en_attente"||r.statut==="pending");
    const matchClinik = filtreClinik === "tous" ? true : String(r.clinique_id) === filtreClinik;
    return matchStatut && matchClinik;
  });

  const statutColor = s => s==="confirme"||s==="confirmed"||s==="tenu" ? "#0A8F58"
    : s==="annule"||s==="cancelled" ? "#E11D48" : "#F59E0B";
  const statutLabel = s => s==="confirme"||s==="confirmed"||s==="tenu" ? "Tenu"
    : s==="annule"||s==="cancelled" ? "Annulé" : "En cours";

  return (
    <div>
      <PageHeader title="📅 Gestion RDV Patients" subtitle="Suivi des rendez-vous pris via MediConnect RDV" />

      {/* KPIs globaux */}
      <Grid cols={4} gap={14} style={{marginBottom:24}}>
        {[
          ["Total RDV", rdvTotal,   "Tous statuts", "#2563EB", "📅"],
          ["RDV Tenus",  rdvTenus,  "Confirmés",    "#0A8F58", "✅"],
          ["Annulés",    rdvAnnules,"Annulés",       "#E11D48", "❌"],
          ["En cours",   rdvEnCours,"En attente",    "#F59E0B", "⏳"],
        ].map(([l,v,sub,c,icon])=>(
          <div key={l} style={{background:"#141E2B",border:`2px solid ${c}30`,borderRadius:14,padding:"20px",textAlign:"center"}}>
            <div style={{fontSize:28,marginBottom:8}}>{icon}</div>
            <div style={{fontSize:11,color:"#4E657A",marginBottom:6,textTransform:"uppercase",letterSpacing:".5px"}}>{l}</div>
            <div style={{fontSize:36,fontWeight:900,color:c,marginBottom:4}}>{v}</div>
            <div style={{fontSize:11,color:"#4E657A"}}>{sub}</div>
            {rdvTotal>0 && <div style={{fontSize:12,color:c,fontWeight:700,marginTop:4}}>{Math.round(v/rdvTotal*100)}%</div>}
          </div>
        ))}
      </Grid>
      ...
      {/* Stats par clinique */}
      <div style={{background:"#141E2B",border:"1px solid #1E2F42",borderRadius:14,padding:20,marginBottom:24}}>
        <div style={{fontSize:14,fontWeight:800,color:"#F0F4F8",marginBottom:16}}>🏥 RDV par établissement</div>
        {isLoading ? <Loader /> : statsParClinique.length === 0 ? (
          <Empty icon="🏥" title="Aucun établissement" />
        ) : (
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead>
              <tr style={{borderBottom:"2px solid #1E2F42"}}>
                {["Établissement","Total RDV","RDV Tenus","Annulés","En cours","Taux tenu"].map(h=>(
                  <th key={h} style={{textAlign:"left",padding:"10px 12px",fontSize:11,color:"#4E657A",fontWeight:700,textTransform:"uppercase",letterSpacing:".5px"}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {statsParClinique.sort((a,b)=>b.total-a.total).map((c,i)=>(
                <tr key={c.id||i} style={{borderBottom:"1px solid #0E1620",cursor:"pointer"}}
                  onMouseOver={e=>e.currentTarget.style.background="#1A2535"}
                  onMouseOut={e=>e.currentTarget.style.background="transparent"}
                  onClick={()=>setFiltreClinik(String(c.id)===filtreClinik?"tous":String(c.id))}>
                  <td style={{padding:"12px",fontSize:13,fontWeight:700,color:"#F0F4F8"}}>
                    {String(c.id)===filtreClinik && <span style={{color:"#2563EB",marginRight:6}}>▶</span>}
                    {c.nom||"Clinique"}
                    <div style={{fontSize:11,color:"#4E657A",fontWeight:400}}>{c.ville||""}</div>
                  </td>
                  <td style={{padding:"12px",fontSize:16,fontWeight:800,color:"#2563EB"}}>{c.total}</td>
                  <td style={{padding:"12px",fontSize:15,fontWeight:700,color:"#0A8F58"}}>{c.tenus}</td>
                  <td style={{padding:"12px",fontSize:15,fontWeight:700,color:"#E11D48"}}>{c.annules}</td>
                  <td style={{padding:"12px",fontSize:15,fontWeight:700,color:"#F59E0B"}}>{c.enCours}</td>
                  <td style={{padding:"12px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <div style={{flex:1,height:6,background:"#0E1620",borderRadius:3,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${c.total>0?Math.round(c.tenus/c.total*100):0}%`,background:"#0A8F58",borderRadius:3}}/>
                      </div>
                      <span style={{fontSize:12,color:"#0A8F58",fontWeight:700,minWidth:36}}>{c.total>0?Math.round(c.tenus/c.total*100):0}%</span>
                    </div>
                  </td>
      ...
      <div style={{background:"#141E2B",border:"1px solid #1E2F42",borderRadius:14,padding:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
          <div style={{fontSize:14,fontWeight:800,color:"#F0F4F8"}}>
            📋 Liste des RDV
            <span style={{fontSize:12,color:"#4E657A",fontWeight:400,marginLeft:8}}>({rdvsFiltres.length} résultat{rdvsFiltres.length>1?"s":""})</span>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {/* Filtre statut */}
            {[["tous","Tous","#8BA0B5"],["tenus","Tenus","#0A8F58"],["encours","En cours","#F59E0B"],["annules","Annulés","#E11D48"]].map(([v,l,c])=>(
              <button key={v} onClick={()=>setFiltreStatut(v)}
                style={{background:filtreStatut===v?c+"20":"#0E1620",border:`1px solid ${filtreStatut===v?c:"#1E2F42"}`,color:filtreStatut===v?c:"#8BA0B5",borderRadius:20,padding:"4px 14px",fontSize:12,cursor:"pointer",fontFamily:"inherit",fontWeight:filtreStatut===v?700:400}}>
                {l}
              </button>
            ))}
            {filtreClinik!=="tous" && (
              <button onClick={()=>setFiltreClinik("tous")}
                style={{background:"rgba(37,99,235,.1)",border:"1px solid #2563EB",color:"#2563EB",borderRadius:20,padding:"4px 14px",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>
                ✕ {cliniques.find(c=>String(c.id)===filtreClinik)?.nom||"Clinique"}
              </button>
            )}
          </div>
        </div>
        ...
        {isLoading ? <Loader /> : rdvsFiltres.length === 0 ? (
          <Empty icon="📅" title="Aucun RDV" subtitle="Modifiez les filtres pour voir des résultats" />
        ) : (
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead>
              <tr style={{borderBottom:"2px solid #1E2F42"}}>
                {["N° RDV","Médecin","Établissement","Date","Statut"].map(h=>(
                  <th key={h} style={{textAlign:"left",padding:"8px 10px",fontSize:11,color:"#4E657A",fontWeight:700,textTransform:"uppercase",letterSpacing:".5px"}}>{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {rdvsFiltres.map((rdv,i)=>{
                ...
                return (
                  <tr key={rdv.id||i} style={{borderBottom:"1px solid #0E1620"}}>
                    ...
                    <td style={{padding:"10px",fontSize:13,color:"#F0F4F8"}}>
                      Dr. {rdv.medecin_prenom||""} {rdv.medecin_nom||rdv.medecin||"—"}
                      {rdv.specialite && <div style={{fontSize:11,color:"#4E657A"}}>{rdv.specialite}</div>}
                    </td>
                    <td style={{padding:"10px",fontSize:12,color:"#8BA0B5"}}>{clinique?.nom||rdv.clinique_nom||"—"}</td>
                    <td style={{padding:"10px",fontSize:12,color:"#8BA0B5"}}>
                      {rdv.creneau ? new Date(rdv.creneau.split(" ")[0]).toLocaleDateString("fr-CI",{day:"numeric",month:"short",year:"numeric"})+" "+(rdv.creneau.split(" ")[1]||"") : rdv.date||"—"}
                    </td>
                    <td style={{padding:"10px"}}>
                      <span style={{fontSize:11,background:sc+"20",color:sc,borderRadius:20,padding:"3px 10px",fontWeight:700}}>{sl}</span>
